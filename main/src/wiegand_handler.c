...
static void wiegand_isr(void* arg)
{
    BaseType_t xTaskWokenByReceive = pdFALSE;
    uint8_t input_read[2] = {gpio_get_level(WIEGAND_D0), gpio_get_level(WIEGAND_D1)};
    xQueueSendFromISR(wiegand_reader_queue, &input_read, &xTaskWokenByReceive);
...
void wiegand_read(void *arg)
{
    ...
    while(1)
    {
        if(xQueueReceive(wiegand_reader_queue, &input_read, 200/portTICK_PERIOD_MS))
        {
